function visualize_PCCosine_imageEvol_sparse(S,mtg_size,indices,figh,figh2)
for iTr = 1:numel(S)
    if nargin==1,  
    ...
    %%
    figh = figure(figh);
    montage(best_imnam_col(1:end-1),'Size',mtg_size,'Indices',indices)
    set(figh,'pos',[511    72   970   900])
    title(compose("%s",S(iTr).meta.explabel),'interpreter','none','FontSize',12)
    saveallform(S(iTr).meta.figdir,"Image_Evol_per_gen_sparse",figh,["jpg","pdf"])
    ...
    [scoreframe_imgs, Clim] = score_frame_image_arr(best_imnam_col(1:end-1),...
                                            meanscores(1:end-1));
    montage(scoreframe_imgs,'Size',mtg_size,'Indices',indices)
    ...
    title(compose("%s",S(iTr).meta.explabel),'interpreter','none','FontSize',12)
    saveallform(S(iTr).meta.figdir,"Image_Evol_per_gen_score_framed_sparse",figh2,["jpg","pdf"])
