function [meta_sing, rasters_sing, Trials_sing] = Evol_BigGAN_FC6_find_singular_exps(meta_new, rasters_new, Trials_new)
...
assert(pref_chan_id(1)==pref_chan_id(2))
    ...
    fprintf('Error trace:\n%s\n',err.getReport);
%     disp(meta)
    meta_sing{end+1} = meta;
    rasters_sing{end+1} = rasters;
    Trials_sing{end+1} = Trials;
    continue
end
