function Evol_Stats_Anim_fun(Stats, EStats, Animal, Expi, option)
global G
if nargin==4
...
evol_stim_fr = cellfun(@(psth)mean(movmean(psth,mvWdL,2),3),EStats(Expi).evol.psth,'UniformOutput',false);
evol_stim_fr = cell2mat(reshape(evol_stim_fr,1,1,[]));
evol_stim_sem = cellfun(@(psth)std(movmean(psth,mvWdL,2),0,3)/sqrt(size(psth,3)),EStats(Expi).evol.psth,'UniformOutput',false);
