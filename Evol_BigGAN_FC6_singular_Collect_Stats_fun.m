function BFEStats = Evol_BigGAN_FC6_singular_Collect_Stats_fun(meta_new, rasters_new, Trials_new)
...
if strcmp(meta.ephysFN, 'Alfa-16112020-001'), continue; 
% this exp loading is incorrect. need to reload.  `201116_Alfa_generate_BigGAN(1)`
end
assert(thread_num==1,"it's 2 threads")
...
%     
%     fprintf("\n\nFound singular experiment %s %s\n",meta.ephysFN, meta.expControlFN);
%     fprintf('Error message:\n%s\n',err.message);
%     fprintf('Error trace:\n%s\n',err.getReport);
%     continue
% end
% fprintf("This is successful!\n")
...
if isfield(Trials.TrialRecord.User, "space")
    ...
    elseif strcmp(meta.ephysFN,'Beto-20072020-003') % old version exp
    BFEStats(iTr).evol.space_names = {'fc6'};% space names and settings
    BFEStats(iTr).evol.space_cfg = {{'fc6',[]}};
    elseif strcmp(meta.ephysFN,'Alfa-21072020-002') % old version exp
    BFEStats(iTr).evol.space_names = {'fc6'};% space names and settings
    BFEStats(iTr).evol.space_cfg = {{'fc6',[]}};
    elseif strcmp(meta.ephysFN,'Alfa-21072020-007') % old version exp
    BFEStats(iTr).evol.space_names = {'fc6'};% space names and settings
    BFEStats(iTr).evol.space_cfg = {{'fc6',[]}};
    else,keyboard;
    end
...
fprintf("ExpStats saved to %s EvolStat.mat!\n",figdir)
    ...
    fprintf("\n\n Experiment processing error %s %s\n",meta.ephysFN, meta.expControlFN);
    fprintf('Error message:\n%s\n',err.message);
    fprintf('Error trace:\n%s\n',err.getReport);
    continue
end
