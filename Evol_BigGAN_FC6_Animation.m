...
fprintf("Processing BigGAN-FC6 Evolution (with hessian selectivity afterward) Exp %d\n",Expi)
thread_n = HEStats(Expi).evol.thread_num;
% ui = HEStats(Expi).evol.unit_in_pref_chan(1);
% assert(all(ui==HEStats(Expi).evol.unit_in_pref_chan))
% the following snippet is to get rid of 0 unit (null channel)
prefchan_id = find((HEStats(Expi).units.spikeID == HEStats(Expi).evol.pref_chan(1)));
chid = find((HEStats(Expi).units.unit_num_arr == 1) & (HEStats(Expi).units.spikeID == HEStats(Expi).evol.pref_chan(1)));
ui = find(prefchan_id==chid);
Window = 50:200;
imgColl = repmat("", HEStats(Expi).evol.block_n-1,thread_n);
scoreColl = zeros(HEStats(Expi).evol.block_n-1,thread_n);
meanscoreColl = zeros(HEStats(Expi).evol.block_n-1,thread_n);
for thr_i = 1:thread_n
for blocki = 1:HEStats(Expi).evol.block_n-1
    gen_scores = squeeze(mean(HEStats(Expi).evol.psth{thr_i,blocki}(ui,Window,:),[1,2]));
    [maxScore, maxIdx] = max(gen_scores); % Idx in local generation
    imgidx = HEStats(Expi).evol.idx_seq{thr_i,blocki}(maxIdx);
    imgfullfn = ls(fullfile(HEStats(Expi).meta.stimuli, HEStats(Expi).imageName(imgidx)+"*"));
    assert(~isempty(imgfullfn), "Image not found %s",fullfile(HEStats(Expi).meta.stimuli, HEStats(Expi).imageName(imgidx)+"*"))
    imgColl(blocki,thr_i) = fullfile(HEStats(Expi).meta.stimuli, imgfullfn); % this is a temporary solution. the suffix may not be .bmp % FIXED @ Jan.29
    scoreColl(blocki,thr_i) = maxScore;
    meanscoreColl(blocki,thr_i) = mean(gen_scores);
end
end
%% Generation averaged psth and sem
evol_stim_fr = cellfun(@(psth)mean(psth,3),HEStats(Expi).evol.psth,'UniformOutput',false);
evol_stim_fr = cell2mat(reshape(evol_stim_fr',1,1,[],thread_n));
evol_stim_sem = cellfun(@(psth)std(psth,0,3)/sqrt(size(psth,3)),HEStats(Expi).evol.psth,'UniformOutput',false);
evol_stim_sem = cell2mat(reshape(evol_stim_sem',1,1,[],thread_n));
% Scalor score for evolution
score_avg = cellfun(@(psth)mean(psth(:,51:200,:),'all') - mean(psth(:,1:50,:),'all'),HEStats(Expi).evol.psth);
score_sem = cellfun(@(psth)std(squeeze(mean(psth(:,51:200,:),[1,2])))...
    /sqrt(size(psth,3)),HEStats(Expi).evol.psth); % - mean(psth(:,1:50,:),[1,2])
%% Generate Movies
...
color_seq = HEStats(Expi).color_seq;
...
title(compose("%s",HEStats(Expi).evol.optim_names(1)))
...
title(HEStats(Expi).evol.optim_names(1)+" Evol Traj");
...
title(compose("%s",HEStats(Expi).evol.optim_names(2)))
ax3_2 = subtightplot(2,4,4,0.07);
shadedErrorBar([],score_avg(2,1:end-1),score_sem(2,1:end-1),...
        'lineprops',{'Color','k','LineWidth',1.5},'transparent',1,'patchSaturation',0.15);%'lineprops',{'Color',[color_seq(blocki, :),0.85]},
xlabel("Generations");ylabel("Response fr (Hz)");axis tight;ylim(scoreYLIM)
title(HEStats(Expi).evol.optim_names(2)+" Evol Traj");
...
ST = suptitle(compose("%s BigGAN FC6 Evol Exp %02d pref chan %s", ...
    Animal, Expi, HEStats(Expi).units.unit_name_arr(HEStats(Expi).units.pref_chan_id)));
%
for blocki = 1:HEStats(Expi).evol.block_n-1
