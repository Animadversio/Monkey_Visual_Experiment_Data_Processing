function [meta,rasters,lfps,Trials] = loadExperiments_tabrows(tabrows, no_return, no_lfp, no_conv)
...
iExp = 0;
for iExp = 1:size(tabrows,1)
    preMeta(iExp).ephysFN = tabrows.ephysFN{iExp}; 
    preMeta(iExp).expControlFN = tabrows.expControlFN{iExp}; % 
    preMeta(iExp).stimuli = tabrows.stimuli{iExp} ;
    preMeta(iExp).comments = tabrows.comments{iExp};
