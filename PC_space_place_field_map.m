...
for Expi = 1:10
% meta = meta_new{2*(Expi-21)-1}; % loading data from temporary store
% rasters = rasters_new{2*(Expi-21)-1};
% Trials = Trials_new{2*(Expi-21)-1};
...
for space_i = 1:3
% Collect the statistics and maps 
signif_chan = [];
rsp_mat_arr = {};
signif_F = [];
signif_P = [];
...
figh = 7 + space_i;
%%
figure(figh);clf;set(figh,'position',[42,          42,        figwidth,       1000 ])
...
title(t,Exp_label)
%%
saveas(figh, fullfile(savepath, sprintf("Exp%02d_%s_placemap.jpg", Expi, space_list{space_i})))
saveas(figh, fullfile(result_dir, sprintf("Exp%02d_%s_placemap.jpg", Expi, space_list{space_i})))
end
end
%%
